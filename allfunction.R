binaryMatrix <- function(
  datapath, # The filepath of EP link files for each cell
  ...
  c1 <- ncol(EPall)
  EPall[,(c1+1):(c1+2)] <- separate(as.data.frame(rownames(EPall)), 1, sep = "#", into = c("gene","eh"))
  EPall[,(c1+3):(c1+5)] <- separate(as.data.frame(EPall$eh), 1, sep = "_", into = c("chr","start","end"))
...
sepBinaryMatrix <- function(
  allEP_binary, # The binary matrix of all EP (the output of step1)
  ...
  for(i in 1:length(gene_list)){
    ...
    if(nrow(EP_matrix) >= 2){
      write.table(EP_matrix, file=paste0(pathout,gene_list[i],".txt"), sep="\t", quote = F, row.names = T, col.names = T)
    }

  progress_value <- (i/length(gene_list))*100
